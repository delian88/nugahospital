    ...
    <section className="relative h-screen min-h-[600px] flex items-center overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-20">
        <div className="max-w-5xl space-y-6 md:space-y-8">
          <div className="inline-flex items-center space-x-2 bg-primary/30 backdrop-blur-md px-4 sm:px-6 py-2 sm:py-2.5 rounded-full border border-white/20">
            <span className="w-2 sm:w-2.5 h-2 sm:h-2.5 bg-primary rounded-full animate-pulse shadow-[0_0_10px_rgba(176,28,116,0.8)]"></span>
            <span className="text-[10px] sm:text-xs font-black text-white uppercase tracking-[0.2em] font-modern">The Future of Health in Abuja</span>
          </div>
          
          <h1 className="text-4xl sm:text-6xl md:text-9xl font-display italic leading-tight text-white drop-shadow-2xl">
            Excellence in <br />
            <span className="shimmer-text font-black not-italic tracking-tighter">Healthcare</span>
          </h1>
          
          <p className="text-lg sm:text-xl md:text-3xl text-slate-100 max-w-3xl leading-relaxed font-modern font-light drop-shadow-lg shimmer-text-white">
          ...
          <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6 pt-6">
            <button className="w-full sm:w-auto bg-primary text-white px-8 sm:px-12 py-4 sm:py-6 rounded-2xl font-black text-base sm:text-lg hover:bg-primary-dark transition-all shadow-2xl shadow-primary/40 transform hover:-translate-y-1 relative overflow-hidden group">
            ...
            <button className="w-full sm:w-auto bg-white/10 backdrop-blur-md text-white border-2 border-white/30 px-8 sm:px-12 py-4 sm:py-6 rounded-2xl font-black text-base sm:text-lg hover:bg-white/20 transition-all transform hover:-translate-y-1">
              Our Departments
            </button>
          </div>

          {/* Slider Indicators */}
          <div className="flex items-center space-x-4 pt-12 sm:pt-16">
            {sliderImages.map((_, index) => (
              <button
                key={index}
                onClick={() => setCurrentSlide(index)}
                className={`h-1 sm:h-1.5 transition-all duration-500 rounded-full ${
                  index === currentSlide ? 'w-12 sm:w-20 bg-primary shadow-[0_0_15px_rgba(176,28,116,1)]' : 'w-4 sm:w-6 bg-white/20 hover:bg-white/40'
                }`}
